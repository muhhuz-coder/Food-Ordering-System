...
def menu():
    # df = pd.read_csv("D:/FAST/Sem5/Database Systems/Project/new project/Db Project/menu.csv")
    ...
    #     curl = mysql.connection.cursor()
    #     curl.execute("INSERT INTO food (food_id,food_name,food_price,category_id, res_id,image_url) values(%s,%s,%s,%s,%s,%s)",
    #         ([count],[name],[price],[category_id],[res_id],[image_url],))
    #     mysql.connection.commit()
    #     count = count+1
    if request.method == 'POST':
        res= request.form.get('res_name')
        curl = mysql.connection.cursor()
        curl.execute("select food_id, food_name, food_price, image_url from food where res_id = (select res_id from restaurant where res_name = %s)",([res]))
        menu_items = curl.fetchall()
        curl.close()
        return render_template('menu.html',params=params, menu_items = menu_items)
    return render_template('login.html',params=params)
...
def showcart():
    ...
    if o_id is None:
        cur.close()
        return render_template('cart.html', params = params, item_records = [], total_price = 0, promo = None, discounted_price = 0)
...
def customization():
    ...
    if o_id is None:
        cur.close()
        return render_template("customization.html", params = params, item_records = [], topping_name = [], topping_price = [], topping_id = [])
